...
void Initialize(void)
{
    ...
    glClearColor(0.4f, 0.8f, 0.4f, 1.0f); //Change the background to green
    if(EnableLight){
...
void WelcomeScreen(){
    ...
    Write("______ Welcome to the Snake Game ______");
    glRasterPos2f(-40, 0);
    Write("______ press 'S' or 's' ,  to start the game ______");
...
void DrawSnake(){
    ...
    ManipulateViewAngle();
      ...
      glColor3f(0.5f, 0.5f, 0.4f);//Orange // sets the color of the plane
    ...
    glutSolidSphere(10,20,20);//Draw the head as a sphere a little bit bigger than the body spheres
    ...
    for(i=0; i<size; i++){//Loop throw the size and draw spheres representing the body
        ...
        glColor3f(0.5f, 0.0f, 0.0f);//Violet
        glScalef(0.5,0.5,0.5);
